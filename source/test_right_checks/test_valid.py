...
class TestPostCharacter:
    ...
    def test_check_added_data_with_different_language(self, post_setup, random_different_language_symbols):
        """
        Тест-кейс на проверку корректности работы системы с различными символами, например с латиницей, кириллицей и знаками пунктуации.
        В рамках теста происходит проверка как по общему списку персонажей (get /characters),
        так и по запросу для одного персонажа (get /characters/{name}).
